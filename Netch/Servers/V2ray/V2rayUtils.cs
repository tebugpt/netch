...
public static class V2rayUtils
{
    public static IEnumerable<Server> ParseVUri(string text)
    {
        ...
        if (text.Contains("?"))
        {
            ...
            if (server.TLSSecureType != "none")
            {
                ...
                server.Fingerprint = parameter.Get("fp") ?? server.Fingerprint;

                var alpn = parameter.Get("alpn");
                if (!string.IsNullOrWhiteSpace(alpn))
                    server.Alpn = Uri.UnescapeDataString(alpn);
                ...
                {
                    server.PublicKey = parameter.Get("pbk") ?? "";
                    server.ShortId = parameter.Get("sid") ?? "";

                    var spx = parameter.Get("spx");
                    server.SpiderX = !string.IsNullOrWhiteSpace(spx) ? Uri.UnescapeDataString(spx) : "/";
                }
            }
        }
    ...
    public static string GetVShareLink(Server s, string scheme = "vmess")
    {
        ...
        if (server.TLSSecureType != "none")
        {
            ...
            if (server.TLSSecureType == "reality")
            {
                parameter.Add("pbk", server.PublicKey ?? "");
                parameter.Add("sid", server.ShortId ?? "");
                parameter.Add("fp", server.Fingerprint ?? "chrome");
                if (!string.IsNullOrWhiteSpace(server.SpiderX) && server.SpiderX != "/")
                    parameter.Add("spx", Uri.EscapeDataString(server.SpiderX));
            }

            if (!server.ServerName.IsNullOrWhiteSpace())
                parameter.Add("sni", server.ServerName!);

            if (!server.Alpn.IsNullOrWhiteSpace())
                parameter.Add("alpn", Uri.EscapeDataString(server.Alpn!));

            if (!server.Fingerprint.IsNullOrWhiteSpace() && server.Fingerprint != "chrome")
                parameter.Add("fp", server.Fingerprint!);

            if (!server.Flow.IsNullOrWhiteSpace())
                parameter.Add("flow", server.Flow!);
