    ...
    private static async Task<Outbound> outbound(Server server)
    {
        ...
        switch (server)
        {
            ...
            case VLESSServer vless:
            {
                    ...
                    new VnextItem
                    {
                        ...
                        users = new[]
                        {
                            new User
                            {
                                ...
                                flow = !string.IsNullOrWhiteSpace(vless.Flow)
                                    ? vless.Flow
                                    : vless.TLSSecureType == "xtls" ? "xtls-rprx-direct" : "",
                ...
                if (vless.TLSSecureType == "xtls" || !string.IsNullOrWhiteSpace(vless.Flow))
                {
                    outbound.mux.enabled = false;
                    outbound.mux.concurrency = -1;
                }
            ...
            switch (server.TLSSecureType)
            {
                ...
                case "reality":
                    streamSettings.realitySettings = new RealitySettings
                    {
                        ...
                        publicKey = server.PublicKey.ValueOrDefault() ?? "",
                        shortId = server.ShortId.ValueOrDefault() ?? "",
                        spiderX = server.SpiderX.ValueOrDefault() ?? "/"
                    };
                    break;
            }
        }
